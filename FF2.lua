...
mainFrame.Size = UDim2.new(0,400,0,400) -- square shape
...
-- Utility functions for toggles/sliders
...
local function loadCatchingPage(page)
    clearContent()
    yOffset = 50 -- leave top space for page buttons

    -- Page switch buttons at top
...
catContent["Visuals"] = function()
    yOffset = 10
    local function createToggleV(name,default,callback)
    ...
    createToggleV("Ball ESP",settings.esp)
    createToggleV("Glow Trail",settings.glowTrail)
    createToggleV("Path Visualizer",settings.path)
    createToggleV("Ball Aura",settings.ballAura)
    createToggleV("Ball Shadow",settings.ballShadow)
    createToggleV("Blue Round",settings.blueRound,function(state)
        local char = player.Character
        if char and char:FindFirstChild("HumanoidRootPart") then
            if state then
                if not char:FindFirstChild("BlueRound") then
                    local circle = Instance.new("Part",char)
                    circle.Name="BlueRound"
                    circle.Size=Vector3.new(10,0.5,10)
                    circle.Anchored=true
                    circle.CanCollide=false
                    circle.Material=Enum.Material.Neon
                    circle.Color=Color3.fromRGB(0,0,255)
                    circle.CFrame=CFrame.new(char.HumanoidRootPart.Position - Vector3.new(0,char.HumanoidRootPart.Size.Y/2,0))
                    RunService.RenderStepped:Connect(function()
                        if char and char:FindFirstChild("HumanoidRootPart") and circle then
                            circle.Position = char.HumanoidRootPart.Position - Vector3.new(0,char.HumanoidRootPart.Size.Y/2,0)
                        end
                    end)
                end
            else
                local old = char:FindFirstChild("BlueRound")
...
print("Star Hub Menu Loaded - Football Fusion 2 Edition (Square GUI, Catching Page 1 & 2, Top Page Buttons)")
